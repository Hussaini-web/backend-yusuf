  ...
  `);
    ...
    const insertManyUsers = db.transaction((users) => {
        for (const user of users) {
            insertUser.run(user.name, user.email, user.role, user.status, user.lastActive, user.avatar);
        }
    });
  ...
  `);
    ...
    const insertManyOrders = db.transaction((orders) => {
        for (const order of orders) {
            insertOrder.run(order.id, order.user, order.avatar, order.project, order.address, order.date, order.status);
        }
    });
